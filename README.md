...
- **Quick peek:** `bgtail <id>` — condensed newest lines (ANSI stripped, repeats
  ...
  free. Asking for **more lines than last time** widens the view instead of
  polling: you get only the earlier lines you have not seen, so widening costs
  the difference rather than the whole window again. The wake message itself
